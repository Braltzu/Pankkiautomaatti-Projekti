...
void tapahtumat::setWebToken(const QByteArray &newWebToken){
    qDebug()<<"setwebtokeni";
    webtoken=newWebToken;
    qDebug()<<webtoken;
    qDebug()<<"tapahtumat webtoken";
}
void tapahtumat::setidasiakas(const QString &newidasiakas){
...
//------------------------------------------------------------------------------------
void tapahtumat::ReceiveTilitapahtumat(QNetworkReply *reply){
...
void tapahtumat::ReceiveTilitapahtumat(){
    qDebug() << "ReceiveTilitapahtumat is called";
    qDebug() <<"tilitapahtumat painettu";
    QString site_url="http://localhost:3000/tilitapahtumat/"+idasiakas;
    qDebug()<<site_url;
    QNetworkRequest request((site_url));
    qDebug()<<"ennen tapahtumat webtoken";
    QByteArray myToken="Bearer "+webtoken;
    qDebug() << "Web Token used:" << webtoken;
    request.setRawHeader(QByteArray("Authorization"),(myToken));
    qDebug()<<webtoken;
    qDebug()<<"tapahtumat webtoken ohi";
    getManager  = new QNetworkAccessManager(this);
    connect(getManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(ReceiveTilitapahtumat(QNetworkReply*)));
